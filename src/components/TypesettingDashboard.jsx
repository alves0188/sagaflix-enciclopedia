...
export default function TypesettingDashboard({ book, universe, onUpdateBook, onUpdateData }) {
  ...
  const gap = 10; // Espaço sutil entre as páginas de um spread para simular a dobra do livro

  // Atualizar estimativa de páginas
  useEffect(() => {
    const measure = () => {
      if (contentRef.current) {
        // scrollWidth do container de colunas nos diz a largura total
        const totalWidth = contentRef.current.scrollWidth;
        const pageFullWidth = currentFormat.width + gap;
        
        const count = Math.round((totalWidth + gap) / pageFullWidth);
  ...
  const maxViews = Math.ceil((fixedPagesCount + dynamicPagesCount + 1) / 2);

  // Funções de Navegação (Simulando scroll vertical, mas movendo horizontalmente)
  ...
  // Margem de Segurança Visual (15px para dentro da borda do papel)
  ...
  // Calcula a posição do Track (Trilho horizontal) para a view atual
  const getTrackTransform = () => {
    const pageFullWidth = currentFormat.width + gap;
    if (currentView === 0) {
      // View 0: Capa centralizada. O Viewport tem (2 * width + gap). A Capa tem (width).
      const centerOffset = (currentFormat.width + gap) / 2;
      return `translateX(${centerOffset}px)`;
    }
    // View V: Queremos a página (2V - 1) alinhada à esquerda do Viewport
    const targetSlot = 2 * currentView - 1;
    const xPos = targetSlot * pageFullWidth;
  ...
  return (
    <div style={{ 
      ...
      height: '100%', 
      width: '100%',
      backgroundColor: '#2b2b2b', // Fundo escuro atrás do livro
      color: '#333',
      ...
      <div style={{ 
        ...
        <div style={{ padding: '1.5rem', flex: 1, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          {/* Navegação Rápida (Simula Vertical) */}
      ...
      >
        ...
        <div style={{
          width: `${(currentFormat.width * 2) + gap}px`,
          height: `${currentFormat.height}px`,
          position: 'relative'
        }}>
          
          {/* TRILHO DE CONTEÚDO (Move horizontalmente para simular troca de página) */}
          <div style={{ 
            display: 'flex', 
            gap: `${gap}px`, 
            position: 'absolute',
            ...
            {/* SLOTS 4 até N: O Texto Principal (A Mágica das Colunas CSS) */}
            <div style={{ position: 'relative', display: 'flex', flexShrink: 0 }}>
              
              {/* Backdrops (Fundo branco das páginas geradas) */}
              <div style={{ position: 'absolute', top: 0, left: 0, display: 'flex', gap: `${gap}px`, zIndex: 1, pointerEvents: 'none' }}>
              ...
              <div 
                ...
                style={{ 
                  ...
                  // A largura de cada coluna CSS tem que ser exatamente a largura interna da página
                  columnWidth: `${currentFormat.width - (currentFormat.padding * 2)}px`,
                  // O gap entre as colunas CSS precisa compensar os 2 paddings + o gap externo das páginas
                  columnGap: `${gap + (currentFormat.padding * 2)}px`,
                  
                  // A altura fixa empurra o texto para novas colunas
                  height: `${currentFormat.height}px`,
                  boxSizing: 'border-box',
                  paddingTop: `${currentFormat.padding}px`,
                  paddingBottom: `${currentFormat.padding}px`,
                  paddingLeft: `${currentFormat.padding}px`,
                  paddingRight: `${currentFormat.padding}px`, // Um padding virtual garante margens simétricas
                  columnFill: 'auto',
                }}
              >
